...
const checkRecords = ()=>{
    if(localStorage.clickCounterMedium === undefined){
        regresarElemento("contadorClicks").innerHTML = "No hay records...";
    }else{
        regresarElemento("contadorClicks").innerHTML = "Número de clicks realizados: " + localStorage.clickCounterMedium;
    }
}
...
const mediumGhost = document.getElementById('mediumGhost');
...
const record = (currentClicks) =>{
    currentClicks++;
    if(localStorage.clickCounterMedium == 0 || localStorage.clickCounterMedium === undefined){
        localStorage.clickCounterMedium = currentClicks;
    }else{
        if(currentClicks < localStorage.clickCounterMedium){
            localStorage.clickCounterMedium = currentClicks;
        }
    }
...
deleteIcon.addEventListener('click', ()=>{
    window.localStorage.removeItem("clickCounterMedium");
...
helpIcon.addEventListener('click', ()=>{
    ...
    help.style.display = "block";
    let ghost = regresarElemento("mediumGhost");
